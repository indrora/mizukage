...
class RawImage:
    ...
    def to_debayered_numpy(
        ...
        *,
        ...
        Returns:
            ...
            half_res=True:  uint16  (height/2, width/2, 3) — fast subsampling
        ...
        if self.is_mono or self.bayer_r_row is None:
            ...
            return rgb.astype(np.float32) if not half_res else rgb

        r_row = self.bayer_r_row
        r_col = self.bayer_r_col

        if half_res:
            return debayer_half(raw, r_row, r_col)
        return debayer_bilinear(raw, r_row, r_col)
    ...
    def to_png(
        ...
        *,
        ...
        raw=False → 8-bit RGB PNG (demosaiced, scaled from 10-bit to 0–255)
        ...
        if raw:
            ...
            rgb = self.to_debayered_numpy(half_res=half_res, subtract_black=subtract_black)
            # Scale to 8-bit range for standard RGB PNG
            rgb8 = (rgb.astype(np.float32) * (255.0 / white)).clip(0, 255).astype(np.uint8)
    ...
    def to_tiff(
        ...
        *,
        ...
        raw=False → 8-bit RGB TIFF (debayered)
